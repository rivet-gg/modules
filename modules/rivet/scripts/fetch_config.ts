...
export async function run(
	...
	const apiEndpoint = ctx.environment.get(ctx.config.apiEndpointVariable) ??
		ctx.config.apiEndpoint;
	...
	assertExists(
		gameId,
		`Missing environment variable: ${ctx.config.gameIdVariable}`,
	);
	assertExists(
		environmentId,
		`Missing environment variable: ${ctx.config.environmentIdVariable}`,
	);
