...
- `RIVET_TAGS` All of the lobby's tags (as described above), in the form of JSON.

As well as a corresponding environment variable in the form `RIVET_TAGS_your_tag=your_tag_value`
for each of the lobby's tags. If you have the tag `foo` set to the value `bar` on a given lobby,
that lobby will have the environment variable `RIVET_TAGS_foo` set to `bar`.
