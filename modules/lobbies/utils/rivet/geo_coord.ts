...
import { EMPTY_GEO_COORDS, RegionGeoCoords } from "../region.ts";

type TraceEntry = ScriptContext["trace"]["entries"][0];
type TraceEntryTypeHttpRequest = Extract<
	TraceEntry["type"],
	{ httpRequest: any }
>["httpRequest"];
export const getRequestGeoCoords = (
	...
	const coords = httpReq.headers["x-backend-client-coords"].split(",").map(
		(e) => parseFloat(e.trim()),
	);
