import { REGIONS as TEST_REGIONS } from "./lobby/backend/test.ts";
import { REGIONS as SERVER_REGIONS } from "./lobby/backend/server.ts";
...
function getDistance(a: RegionGeoCoords, b: RegionGeoCoords) {
	...
	const x = sinOfDLat * sinOfDLat +
		sinOfDLon * sinOfDLon * Math.cos(aLatitude) * Math.cos(bLatitude);
