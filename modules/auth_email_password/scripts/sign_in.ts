...
import {
	ensureNotAssociated,
	ensureNotAssociatedAll,
} from "../utils/link_assertions.ts";
...
export type Request =
	...
	& (ConnectRequest | SignInRequest | SignUpRequest);
...
export async function run(ctx: ScriptContext, req: Request): Promise<Response> {
	...
	if ("connectEmail" in req) {
		...
		try {
			...
			if (
				!(e instanceof RuntimeError) || e.code !== "identity_provider_not_found"
			) {
				throw e;
			}
