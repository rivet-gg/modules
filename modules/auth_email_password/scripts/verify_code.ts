...
async function doPasswordAction(
	ctx: ScriptContext,
	userId: string,
	data: object,
) {
	...
	if ("newHash" in data && typeof data.newHash === "string") {
		...
		await ctx.modules.userPasswords.setRawHash({
			userId,
		...
		if (
			new Date(hashMeta.updatedAt).getTime() >=
...
export async function run(ctx: ScriptContext, req: Request): Promise<Response> {
	...
	const { data, succeeded } = await ctx.modules.verifications.attempt({
		token: req.token,
		code: req.code,
	});
