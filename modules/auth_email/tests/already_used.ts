...
async function signUpEmailPass(
	...
	const { verificationId, code } = await getVerification(ctx, email);
	return await ctx.modules.authEmail.signUpEmailPass({
		verificationId,
		code,
...
test("sign_up_with_email_no_pass_then_pass", async (ctx: TestContext) => {
	...
	await signUpEmailNoPass(ctx, email);
	{
		const { verificationId, code } = await getVerification(ctx, email);
		const error = await assertRejects(() => {
			return ctx.modules.authEmail.signUpEmailPass({
				verificationId,
				code,
				email,
				password,
