...
export async function handler(ctx: Context, req: Request): Promise<Response> {
    ...
    ctx.postgres.transaction("accept_request", async tx => {
        ...
        if (friendRequest.target_user_id !== userId) throw new Error("You are not the receiver of this friend request");
        if (friendRequest.accepted_at) throw new Error("Friend request already accepted");
        ...
        const [userIdA, userIdB] = [friendRequest.sender_user_id, friendRequest.target_user_id].sort();
        tx.queryObject`INSERT INTO friends (user_id_a, user_id_b) VALUES (${userIdA}, ${userIdB})`;
    });
