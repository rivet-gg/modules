...
export function getClient(cfg: FullConfig, provider: string) {
	...
	if (!providerCfg) {
		throw new RuntimeError("invalid_provider", { statusCode: 400 });
	}

	// TODO: Remove hardcoded origin
	const redirectUri = new URL(
		`http://localhost:6420/modules/auth_oauth2/route/callback/${provider}`,
	).toString();
...
export async function getUserUniqueIdentifier(
	accessToken: string,
