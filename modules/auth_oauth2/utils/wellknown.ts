 ...
 */
export async function getFromOidcWellKnown(
	wellKnownUrl: string,
): Promise<ProviderEndpoints> {
	const res = await fetch(wellKnownUrl).catch(() => {
	...
	if (typeof userinfo_endpoint !== "string") {
		throw new RuntimeError("invalid_config");
	}
	if (!Array.isArray(scopes_supported)) {
		throw new RuntimeError("invalid_config");
	}
	if (scopes_supported.some((scope) => typeof scope !== "string")) {
		throw new RuntimeError("invalid_config");
	}
