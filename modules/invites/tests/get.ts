...
test("get senders", async (ctx: TestContext) => {
	...
	const { user: recipient, token: recipientToken } = await ctx.modules.users.register({
		username: faker.internet.userName(),
		identity: { guest: {} },
	});
	...
	assertGreater(senderAsSenderGetRes.invites.length, 0, "No invites were found");
	const senderInvite = senderAsSenderGetRes.invites[0];

	assertEquals(senderInvite.from, sender.id, "Invite sender id was not correct");
	assertEquals(senderInvite.to, recipient.id, "Invite recipient id was not correct");
	...
	assertGreater(recipientAsRecipientGetRes.invites.length, 0, "No invites were found");
	const recipientInvite = recipientAsRecipientGetRes.invites[0];

	assertEquals(recipientInvite.from, sender.id, "Invite sender id was not correct");
	assertEquals(recipientInvite.to, recipient.id, "Invite recipient id was not correct");
	...
	assertEquals(senderGetAll.invites, recipientGetAll.invites, "Invites did not match between sender and recipient");
});
