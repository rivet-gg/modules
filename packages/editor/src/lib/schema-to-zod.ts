...
import { AnySchemaElement, is } from "../../../toolchain/build/schema/schema";

export function convertSchemaToZod(schema: AnySchemaElement): ZodType<any> {
    ...
    if (is("intersection", schema)) {
        return z.intersection(convertSchemaToZod(schema.left), convertSchemaToZod(schema.right));
    }
    if (is("union", schema)) {
    ...
    if (is("object", schema)) {
        return z.object(
            Object.fromEntries(
                Object.entries(schema.properties).map(([key, value]) => [key, convertSchemaToZod(value)]),
            ),
        ).strict();
    }
    if (is("record", schema)) {
