...
async function compileTypeHelpers(registry: Registry) {
	...
	for (const module of registry.modules.values()) {
		...
		for (const script of module.scripts.values()) {
			...
			const importPath = path.join(registry.path, "modules", module.name, "scripts", `${script.name}.ts`);
			scripts.push(`
// ${module.name}/${script.name}
import type { Request as ${scriptId}Req, Response as ${scriptId}Res } from ${JSON.stringify(importPath)};
interface ${module.name}Module {
	${script.name}: {
		request: ${scriptId}Req;
		response: ${scriptId}Res;
	}
}
			`.trim());
		}
