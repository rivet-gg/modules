import { ScriptContext } from "@ogs/helpers/friends/list_incoming_friend_requests.ts";
...
export async function handler(
	...
	const query = await ctx.postgres.run((conn) =>
        ...
        WHERE target_user_id = ${userId} AND accepted_at IS NULL AND declined_at IS NULL
