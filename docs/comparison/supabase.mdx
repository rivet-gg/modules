...
- **OpenGB Engine** Abstracts away backend complexity to make writing [modules](/modules) simple & fast
- **OpenGB Module Registry** Pre-made [modules](/modules) that can be used to get up and running instantly

OpenGB's modularity lets you use pre-existing [modules](/modules) to get started faster, use modules from the community, and share modules across multiple games.  This allows it to be easily customized and extended to fit the needs of a game.
...
Open Game Backend instead provides these same features as [modules](/modules) that can be modified & extended as needed. For example: authentication and storage modules. These can be customized easily by running the `opengb fork` command.
