...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', marginBottom: '4px', gridColumn: 'span 2' }}>
  ...
  <div style={{ display: 'flex' }}>
    ...
    <div style={{ width: "100%" }}><div>Run the following command in your project directory:</div><div style={{ marginTop: '-8px', marginBottom: '-32px' }}><CodeBlock>opengb module add lobbies</CodeBlock></div></div>
  </div>
</div>
...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', marginBottom: '4px' }}>
  ...
  <div style={{ display: 'flex' }}>
    ...
    <div style={{ width: "100%" }}><a href="https://github.com/NathanFlurry">NathanFlurry</a>, <a href="https://github.com/rivet-gg">rivet-gg</a></div>
  </div>
</div>
...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', marginBottom: '4px' }}>
  ...
  <div style={{ display: 'flex' }}>
    ...
    <div style={{ width: "100%" }}><a href="https://github.com/rivet-gg/opengb-modules/tree/main/modules/lobbies">View Source Code</a></div>
  </div>
</div>
...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', marginBottom: '4px' }}>
  ...
  <div style={{ display: 'flex' }}>
    ...
    <div style={{ width: "100%" }}>1 actor</div>
  </div>
</div>
...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', marginBottom: '4px' }}>
  ...
  <div style={{ display: 'flex', flexDirection: 'column' }}>
    <a href="/modules/lobbies/scripts/create" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Create Lobby</a><a href="/modules/lobbies/scripts/destroy" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Destroy Lobby</a><a href="/modules/lobbies/scripts/fetch_lobby_manager_state" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Fetch Lobby Manager State</a><a href="/modules/lobbies/scripts/find" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Find Lobby</a><a href="/modules/lobbies/scripts/find_or_create" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Find Or Create Lobby</a><a href="/modules/lobbies/scripts/join" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Join Lobby</a><div style={{ fontWeight: 'normal' }}><em>...and 5 more</em></div>
  </div>
</div>
  </div>
</div>
...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', marginBottom: '4px' }}>
  ...
  <div style={{ display: 'flex' }}>
    ...
    <div style={{ width: "100%" }}>
  <div style={{ display: 'flex', flexDirection: 'column' }}>
    <a href="/modules/lobbies/scripts/force_gc" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>Force Garbage Collection</a><a href="/modules/lobbies/scripts/list_regions" style={{ display: 'block', textDecoration: 'none', color: 'inherit', border: 'none', fontWeight: 'normal' }}>List Regions</a>
  </div>
</div>
  </div>
</div>
...
The Lobbies system is tightly integrated with Dynamic Servers to automatically boot lobbies on-demand when players need them. Read more about how Dynamic Servers operates your game servers [here](/dynamic-servers/index).
...
Lobby tags are an optional property that can be passed to the [`lobbies.find`](/matchmaker/api/lobbies/find) and [`lobbies.create`](/matchmaker/api/lobbies/create) endpoints to allow filtering which lobbies the matchmaker will route a player to.
...
When passed to [`lobbies.find`](/matchmaker/api/lobbies/find) the matchmaker will only return lobbies that have a **superset** of the given tags.
...
Here is what would happen with the following [`lobbies.find`](/matchmaker/api/lobbies/find) requests:

<Note>
  This example uses
  [`prevent_auto_create_lobby`](http://localhost:3000/docs/matchmaker/api/lobbies[`lobbies.find`](/matchmaker/api/lobbies/find)#prevent-auto-create-lobby)
...
1. **A [`lobbies.find`](/matchmaker/api/lobbies/find) request with no tags**

   The matchmaker would connect the player to any of the 3 lobbies.

2. **A [`lobbies.find`](/matchmaker/api/lobbies/find) request with the tags `map = sandstorm`**

   The matchmaker would connect the player to either lobby 1 or lobby 2.

3. **A [`lobbies.find`](/matchmaker/api/lobbies/find) request with the tags `map = sandstorm`, `worldId = 123abc`**

   The matchmaker would only connect players to lobby 1.

4. **A [`lobbies.find`](/matchmaker/api/lobbies/find) request with the tags `map = sandstorm`, `worldId = foobar`, `difficulty = hard`**
...
5. **A [`lobbies.find`](/matchmaker/api/lobbies/find) request with the tags `map = tropic`**

   The matchmaker would error because there are no lobbies with the given tags.

### Authentication

It can be useful to allow/disallow certain tags from being used when creating or finding lobbies. This can be done via [custom external verification](/matchmaker/guides/anti-botting#custom-external-verification).
...
[`lobbies.find`](/matchmaker/api/lobbies/find), [`lobbies.join`](/matchmaker/api/lobbies/join), and
[`lobbies.create`](/matchmaker/api/lobbies/create) will fail if captcha is not provided when requested.
...
captcha. After a successful captcha completion, retry the [`lobbies.find`](/matchmaker/api/lobbies/find),
[`lobbies.join`](/matchmaker/api/lobbies/join), or [`lobbies.create`](/matchmaker/api/lobbies/create)
...
endpoint after every [`lobbies.find`](/matchmaker/api/lobbies/find), [`lobbies.join`](/matchmaker/api/lobbies/join),
or [`lobbies.create`](/matchmaker/api/lobbies/create) request.

### Configuration changes

In this example, Rivet will send `https://my.app/verify` an HTTP POST request
after every [`lobbies.join`](/matchmaker/api/lobbies/join) request it
receives.
...
_† See [lobbies.setState](/matchmaker/api/lobbies/set-state)._

_†† See [Lobby environment variables](/matchmaker/concepts/lobby-env) for more info._
...
- Any other status code tells the matchmaker it should reject the request with the [`MATCHMAKER_VERIFICATION_FAILED`](/general/errors/mm/verification-failed) error
...
the user's request will fail with the [`MATCHMAKER_VERIFICATION_FAILED`](/general/errors/mm/verification-failed)
error code. If Rivet's request to your server times out or fails for any reason, the matchmaker request will fail with the
[`MATCHMAKER_VERIFICATION_REQUEST_FAILED`](/general/errors/mm/verification-request-failed) error code.
...
[`lobbies.create`](/matchmaker/api/lobbies/create) endpoint. This guide will discuss in detail all of
...
<Warning>
  ...
  [`lobbies.create`](/matchmaker/api/lobbies/create) request by setting `enable_public` and/or
...
[`lobbies.create`](/matchmaker/api/lobbies/create) endpoint and have it be sent directly to the new
...
<Note>
  ...
  [here](/matchmaker/guides/anti-botting#custom-external-verification).
</Note>
